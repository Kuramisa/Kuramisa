...
export default class KanvasImages {
    ...
    async affect(_image: string) {
        ...
        const bg = await loadImage(`${process.cwd()}/assets/images/affect.png`);
    ...
    async ship(member1: GuildMember, member2: GuildMember) {
        ...
        const user1 = await member1.user.fetch();
        const user2 = await member2.user.fetch();

        const db1 = await database.users.fetch(user1.id);
        const db2 = await database.users.fetch(user2.id);

        const { card: card1 } = db1;
        const { card: card2 } = db2;
        ...
        switch (card1.outlines.type) {
            ...
            case "status": {
                strokeStyle1 = util.member.statusColor(
                    member1.presence?.status
                );
                break;
            }
            case "avatar": {
        ...
        switch (card2.text.type) {
            case "banner": {
                ...
                if (colors) fillStyle2 = util.randomElement(colors);
        ...
        switch (card2.outlines.type) {
            ...
            case "status": {
                strokeStyle2 = util.member.statusColor(
                    member2.presence?.status
                );
                break;
            }
            case "avatar": {
