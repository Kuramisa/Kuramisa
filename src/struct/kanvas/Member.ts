...
export default class KanvasMember {
    ...
    async levelUpCardOld(iMember: GuildMember) {
        ...
        switch (card.text.type) {
            case "banner": {
                ...
                if (colors) fillStyle = randEl(colors);
        ...
        switch (card.outlines.type) {
            case "banner": {
                ...
                if (colors) strokeStyle = randEl(colors);
                else strokeStyle = "#FFA500";
                break;
            }
            case "status": {
                strokeStyle = statusColor(member.presence?.status);
                break;
            }
            case "avatar": {
    ...
    // TODO: finish profile method
    async profile(iMember: GuildMember) {
        const { database, logger, owners } = kuramisa;
    ...
    async rank(iMember: GuildMember) {
        ...
        const currentRank = await xp.getRank(db);
    ...
    async leaderboard(usersCount: number = 10) {
        ...
        for (let i = 0; i <= dbUsers.length; i++) {
            ...
            if (!dbUser) continue;
            let user = kuramisa.users.cache.get(dbUser.id);
            if (!user)
                user = await kuramisa.users
                    .fetch(dbUser.id)
                    .catch(() => undefined);
            if (!user) continue;
            ...
            const currentRank = await xp.getRank(dbUser);
