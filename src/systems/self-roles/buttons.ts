...
export default class SelfRolesButtons {
    async buttonAdd(interaction: ChatInputCommandInteraction) {
        ...
        try {
            ...
            if (latestRow.components.length < 5) {
                const row = new Row();
                for (const component of latestRow.components) {
                    if (component.type !== ComponentType.Button) continue;
                    const newButton = new Button(component.data);
    ...
    async buttonEdit(interaction: ChatInputCommandInteraction) {
        ...
        try {
            ...
            const newButtonStyle = options.getInteger("sr_button_style");
            ...
            if (!dbMessage)
                return interaction.reply({
                    content: bold(
                        "This message does not exist in the database",
                    ),
                    flags: "Ephemeral",
                });
            ...
            for (const row of message.components) {
                ...
                for (const component of row.components) {
                    ...
                    const newButton = Button.from(
                        component.customId === oldId ? button : component,
                        false,
                    );
